---
name: Julia-skills-agent
description: for_hatenablog内のJuliaに関するルールを定めたファイルです。
applyTo: '**/*.jl, **/*.ipynb'
---
  
## Juliaの利用目的とコーディング技術について

### Juliaの利用目的
- 私はJuliaを、主に数値計算や数理最適化モデルの実装に使用します。
- Juliaコードを生成する際には、これらの利用目的に適したライブラリやフレームワークを活用してください。

#### 仮想環境について
- 私はJuliaの仮想環境を使用して、プロジェクトごとに依存関係を管理します。

### Juliaの技術レベルについて
#### 現在のスキルセット
- 私はJuliaに関して中級程度の知識と実装スキルをもっています。
  - 基本的なJuliaの文法を把握しています。
...
- 一方で、Juliaの高度な機能や最適化手法についてはまだ学習中です。
  - クラスやオブジェクト指向プログラミングの理解は限定的です。
  - デコレータやジェネレータなどの高度なJulia機能の使用経験は浅いです。
  - 大規模なプロジェクトでのJuliaの設計パターンやベストプラクティスについても理解は限定的です。
...
### コーディングスタイル
- Juliaのコードを生成する際は、PEP 8のスタイルガイドに準拠したスタイルで書いてください。
- Juliaでの関数やクラスの命名には、PEP 8に従ってください。
...
- Juliaのコードで関数やクラスを定義するときは、`'''`でドキュメンテーション文字列を使用し、関数やクラスの目的や引数、戻り値について簡潔に説明してください。
